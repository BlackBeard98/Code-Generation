...
class TestSum(unittest.TestCase):
    ...
    def test_ModifyMethod(self):
        def func(cmd:ModifyMethodCommand,name):
            return cmd.DecoratedBy("Memoize").WithName(name+"_modified")

        self.Engine.From("src/testcases/ModifyMethod/in.py").Select(ClassDef()).Select(FunctionDef()).Where(lambda x: x.node.name == "fib").Using(lambda x:(x.node.name,"name")).Execute(func)
...
if __name__ == '__main__':
    unittest.main()
    #TestSum("test_ModifyMethod").test_ModifyMethod()
